...
var typeHintSet = [
  ...
  { hint: /^(.*[_ ])?(time|date|fecha)/i, type: VarType.TIME }, // Quite general, eg. matches "Start date (AEST)".
  { hint: /^(year|año)$/i, type: VarType.TIME }, // Match "year" only, not "Final year" or "0-4 years".
  { hint: /^postcode|poa|(.*_code)$/i, type: VarType.ENUM }
];

var endDateHintSet = [
  { hint: /^(.*[_ ])?end[\s_]?(time|date|fecha)/i, type: true } // Matches "end_date" or "My end time (AEST)".
];
