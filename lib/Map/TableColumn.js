...
var typeHintSet = [
  ...
  { hint: /^(address|addr|direccion)$/i, type: VarType.ADDR },
  { hint: /^(.*[_ ])?(depth|height|elevation)$/i, type: VarType.ALT },
  { hint: /^(.*[_ ])?(time|date|fecha|created_at)/i, type: VarType.TIME }, // Quite general, eg. matches "Start date (AEST)".
  { hint: /^(year|año)$/i, type: VarType.TIME }, // Match "year" only, not "Final year" or "0-4 years".
  { hint: /^postcode|poa|(.*_code)$/i, type: VarType.ENUM }
];

var endDateHintSet = [
  { hint: /^(.*[_ ])?end[\s_]?(time|date|fecha|created_at)/i, type: true } // Matches "end_date" or "My end time (AEST)".
];

var subtypeHintSet = [{ hint: /^(.*[_ ])?(year|año)/i, type: VarSubType.YEAR }];
